...
\begin{theorem}\label{th:main} $\sigcnn=\frac{\cscale}{\sqrt{w\wconv}}$  in \Cref{assmp:main}. As $w\ra\infty$, with  $\bcnn = \ \left(\dconv \sigcnn^{2(\dconv-1)}\sigfc^{2\dfc}+\dfc \sigcnn^{2\dconv}\sigfc^{2(\dfc-1)}\right)$ for CNN we have
\begin{align*}
