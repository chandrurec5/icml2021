...
\end{definition}
\textbf{Remark:} A coordinate of NPF can be non-zero only if the corresponding path is active ($A_{\Theta}(x,p)=1$), which happens only if all the gates in the path are $1$. 
...
Let $\nabla_{\Theta}v_{\Theta}=(\varphi_{p,\Theta},p\in[P])$. Then NTK is given by $K_{\Theta}(x,x')=\phi^\top_{x,\Theta}(\nabla_{\Theta}v_{\Theta})^\top(\nabla_{\Theta}v_{\Theta})\phi_{x',\Theta}$.
\end{proposition}
\Cref{prop:ntknew} can be interpreted as the NTK being equal to the inner product of the NPFs with respect to the positive semi-definite matrix $(\nabla_{\Theta}v_{\Theta})^\top(\nabla_{\Theta}v_{\Theta})$ is a $\Pfc\times \Pfc$ matrix which is solely dependent on the weights of the network.
\begin{comment}
