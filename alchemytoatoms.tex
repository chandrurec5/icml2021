...
In this paper,  we further decompose the NPK into \emph{atomic} units in a way that explains the role of weights, activations, width and depth.  We show (i) weights: primary role of weights is to trigger the ReLU \emph{on/off} (i.e., $1/0$),  (ii) activations: each ReLU is a gate which provides a binary feature, (iii) width: stacking ReLUs widthwise in a layer gives rise to a base kernel which measures the \emph{average} number of triggered ReLUs, (iv) depth: stacking layers depthwise gives rise to a \emph{Hadamard product}. 
