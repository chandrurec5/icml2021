...
\begin{abstract}
The question ``\emph{is deep learning alchemy?}" mainly stems from the lack of \emph{pedagogical nuggets} that explain training, generalisation, roles of weights/activation/width/depth of even standard deep neural networks (DNNs).  We address this issue by combining \emph{neural tangent kernel} (NTK) and \emph{dual view} for DNNs with rectified linear units (ReLUs). It is known that training a randomly initialised infinite width DNN is equivalent to a kernel method with the NTK at initialisation. Recent work based on dual view exploited the gating property of ReLU to show that when the weights and the gates are decoupled, the NTK simplifies into a neural path kernel (NPK) which is solely dependent on the information in the gates. Also, it was showed that (i) gates are learnt during training, (ii) learnt gates generalise better and (iii) most information is in the gates.
