...
Recently, \citet{npk} developed a dual view for DNNs with ReLUs. In the dual view, the computations are broken down \emph{path-by-path} as opposed to the \emph{layer-by-layer} primal view where the computations proceed. The dual view is possible thanks to the special gating property of ReLU, i.e., each ReLU is also a gate which either blocks (multiplies by $0$) or allows its pre-activation input (multiplies by $1$). This provides a \emph{sub-network} based interpretation for DNNs: each input has a corresponding \emph{active sub-network} comprising of the gates that are \emph{on} and weights through such gates, that for produces the output. 

They also showed via  theory and experiments that the gates (i.e., NPFs) are central to DNNs with ReLU and with the right NPFs the NPV can be discarded (i.e., retrained). The highlights are presented in the bulleted points below.


Using the dual view \citet{npk} encoded the gates in the so called \emph{neural path feature} (NPV) and the weights in the so called \emph{neural path value} (NPV). 
...
$\bullet$ Just as a single perceptron is characterised by a hyperplane, a DNN (with ReLU) being a multi-layered-perceptron is \emph{almost completely} characterised by the collection of hyperplanes (associated with the ReLUs that form the DNN).
