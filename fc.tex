...
In this section, we simply the NTK into atomic units (\Cref{th:main}): we first simplify the \emph{neural path kernel} (NPK) associated with the neural path features into a product of base kernels units in \Cref{lm:productkernel} (new result in this paper) and invoke {Theorem $5.1$}, \cite{npk}. 
...
$
H^{\text{fc}}_{\Theta}(s,s')= \ip{x_s,x_{s'}}_{\Lambda(\cdot,x_s,x_{s'})}=\ip{x_s,x_s'}\cdot\Pi_{l=1}^{d-1}H^{\text{lyr}}_{l,\Theta}(s,s')
$
}
\end{lemma}
...
\end{align*}
\end{theorem}
In \Cref{th:main} $(a)$ was already shown in {Theorem $5.1$}, \cite{npk}, and $(b)$ follows from \Cref{lm:productkernel}. Also, it is known from prior results \cite{arora2019exac,cao2019generalization} that training and generalisation is characterised by the NTK, and the \Cref{th:main} (as well as {Theorem $5.1$}, \cite{npk}) shows that when learning with fixed gates is characterised by the NPK. The implications of \Cref{th:main} are listed below.
...
$\bullet$ \textbf{Role of Activations:} The ReLU is a special activation, that is, it acts as a gate and the gates themselves are learnt. In particular, learnt gates achieve better test accuracy than random gate (see \Cref{sec:exp}). 

The points on the importance of gates (i.e., NPFs) and the fact that the gates (i.e., NPFs) are learnt were also made in prior work \cite{npk}. In this paper we strengthen these with additional experiments (see \Cref{sec:exp}).

$\bullet$ \textbf{Width} is responsible for averaging (division by $w$) of the base kernels. This is a new observation in this paper.

$\bullet$ \textbf{Depth} is gives rise to a product of base kernels. This is a new observation in this paper.
...
$\bullet$ \textbf{Input Invariance} Even if the input to the value network is set of a constant, i.e., even if $\langle x,x'\rangle$ is made constant, the gates still hold information in the from of $\Pi_{l=1}^{d-1} \frac{H^{\text{lyr}}_{l,\Tf_0}(x,x')}{w}$. We verify this experimentally (see \Cref{sec:exp}).
